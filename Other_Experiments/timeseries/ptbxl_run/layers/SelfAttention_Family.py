...
from .rbnn_layers import LinearRBNN
from .irnet_layers import LinearIRNet
...
class AttentionLayer(nn.Module):
    def __init__(
        ...
        if sum([use_ggd, use_xnor, use_dorefa, use_adabin]) > 1:
            raise ValueError(
                "Only one of use_ggd, use_xnor, use_dorefa, use_rbnn, use_irnet can be True"
            )

        if use_ggd:
            self.query_projection = LinearGGD(d_model, d_keys * n_heads, N_factor=5.0)
            self.key_projection = LinearGGD(d_model, d_keys * n_heads, N_factor=5.0)
            self.value_projection = LinearGGD(d_model, d_values * n_heads, N_factor=5.0)
            self.out_projection = LinearGGD(d_values * n_heads, d_model, N_factor=5.0)
