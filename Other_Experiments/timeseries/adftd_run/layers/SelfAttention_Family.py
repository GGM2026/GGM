...
class AttentionLayer(nn.Module):
    def __init__(
        ...
        if use_ggd:
            self.query_projection = LinearGGD(d_model, d_keys * n_heads, N_scale=5.0)
            self.key_projection = LinearGGD(d_model, d_keys * n_heads, N_scale=5.0)
            self.value_projection = LinearGGD(d_model, d_values * n_heads, N_scale=5.0)
            self.out_projection = LinearGGD(d_values * n_heads, d_model, N_scale=5.0)
        elif use_xnor:
