...
#
# Policy:
#   - Finite-bit GGD only. No mixed/full-FP activation mode.
#   - Fixed random/orthogonal G created once at initialization.
#   - No G resampling and no post-construction N_factor changes.
#   - No activation-aware G, no learned metric, no dot-detached mode,
#     no projection smoothing/noise.
#   - Forward is the Monte Carlo kernel estimator.
#   - Backward is an analytic kernel surrogate:
#       * exact K' table when available,
#       * arcsin derivative for 1-bit when table is disabled,
#       * identity derivative fallback otherwise.
#   - Quantizer geometry table_s0 is used for k_bits >= 2.
#   - For k_bits == 1, the projected quantizer returns exactly +/-1,
#     so table_s0 does not leak into 1-bit amplitude.
#   - Runtime amplitude scaling is separate via scale_policy.
# ============================================================
...
from .seed import make_G_from_seed
...
def _quantize_projected_base(z: torch.Tensor, k_bits: int, table_s0: float) -> torch.Tensor:
    ...
    return ggd_quantize(z, int(k_bits), s0=float(table_s0))
...
def quantize_projected_with_scale(
    ...
    """
    if int(k_bits) >= 16:
        raise ValueError("Clean GGDLinear only supports finite-bit quantized sides.")
...
class LinearGGM(nn.Module):
    ...
    def __init__(
        ...
        if self.k_bits_x >= 16 or self.k_bits_w >= 16:
            raise ValueError("Clean GGDLinear only supports finite-bit quantized sides. Use a separate layer for FP.")
        ...
        if self.table is not None:
            print(
                f"[GGDLinear clean] Exact K/K' table bits=({self.k_bits_x},{self.k_bits_w}) "
            ...
            print(
                f"[GGDLinear clean] Table disabled for bits=({self.k_bits_x},{self.k_bits_w}); "
