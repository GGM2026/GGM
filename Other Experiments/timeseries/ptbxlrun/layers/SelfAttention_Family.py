...
class AttentionLayer(nn.Module):
    def __init__(
        ...
        if use_ggm:
            self.query_projection = LinearGGM(d_model, d_keys * n_heads, N_factor=5.0)
            self.key_projection = LinearGGM(d_model, d_keys * n_heads, N_factor=5.0)
            self.value_projection = LinearGGM(d_model, d_values * n_heads, N_factor=5.0)
            self.out_projection = LinearGGM(d_values * n_heads, d_model, N_factor=5.0)
