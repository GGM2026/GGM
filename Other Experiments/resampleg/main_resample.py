...
class model_ggm(nn.Module):
    def __init__(self, base_seed):
        super().__init__()
        self._resample_rng = random.Random(base_seed + 999)

        self.conv1 = Conv2dGGM(3, 32, kernel_size=5, stride=2, padding=1, bias=False, N_scale=2, g_seed=base_seed)
        self.fc1 = LinearGGM(32 * 15 * 15, 32, bias=False, N_scale=2, g_seed=base_seed + 1)
        self.fc2 = LinearGGM(32, 32, bias=False, N_scale=2, g_seed=base_seed + 2)
        self.fc3 = LinearGGM(32, 32, bias=False, N_scale=2, g_seed=base_seed + 3)
        self.head = LinearGGM(32, 10, bias=False, N_scale=2, g_seed=base_seed + 4)

    def resample_G(self):
        for m in self.modules():
            if isinstance(m, (LinearGGM, Conv2dGGM)):
...
def run_training(cfg: Config, base_seed: int):
    ...
    model = model_ggm(base_seed).to(cfg.device)
